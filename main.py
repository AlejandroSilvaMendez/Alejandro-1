    ...
    try:
        ...
        if '/adduser' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMessage(update.message.chat.id,'⚠No tiene permiso UwU⚠')
            return
        if '/banuser' in msgText:
            ...
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'⚠No se puede banear Onichan⚠')
                        return
                    jdb.remove(user)
                    jdb.save()
                    msg = '⚠ @'+user+' Baneado⚠'
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save()
                   msg = '✅ ZIP '+ sizeof_fmt(size*1024*1024)+' establecidos'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        if '/uptype' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠Error en el comando /uptype (typo de subida (evidence,draft,blog,calendar))⚠')
            return
        if '/proxy' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            ...
            bot.editMessageText(message,start_msg)
             ...
             if loged:
                ...
                bot.editMessageText(message,'⚠Error Onichan⚠\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
             pass
            ...
            #    
                # set in debug
            #    api_id = 7375053
            #    api_hash = '78d1c032f3aa546ff5AWDAWDAff0e7f341'
            #    bot_token = '5124841893:AAH30p6AWDAWDAzi2oPlaZwBmCfWQ1KelC6KUg'
...
def main():
    ...
    #set in debug
    bot_token = 5204210218:AAH83ASbUsDf4pmnJa_AunXwN63q91HUvJc
